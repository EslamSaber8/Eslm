...
exports.createOfferValidator = [
    ...
    check('user').isMongoId().withMessage('Invalid Offer  id format'),
    ...
    .custom((val, { req }) =>
      ...
      Offer.findOne({ user: req.user._id, report: req.body.report }).then(
        (offer) => {
          console.log(offer);
          if (offer) {
            return Promise.reject(
              new Error('You already created a offer before')
            );
          }
        }
      )
